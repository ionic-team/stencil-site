...
<p><strong>Note:</strong> in HTML, you must set attributes using dash-case:</p>
...
<p>It&#39;s important to know, that <code>@Prop</code> is immutable from inside the component logic. Once a value is set by a user, the component cannot update it internally.</p>
...
<p>To do validation of a prop, you can use the <code>Watch</code> decorator:</p>
...
<p>In the above example, we set <code>this.completedTodos</code> to a new array made up of the existing <code>completedTodos</code> and our new <code>todo</code>.</p>
<p>This calls the <code>completedTodos</code> setter, which triggers the re-render.</p>
...
<p><stencil-route-link url="/docs/templating" router="#router" custom="true">
