...
export default function ResourcesPage(): JSX.Element {
  ...
  const [activeTab, setActiveTab] = useState('article-blogs');
  console.log(styles);

  const activateTab = (ev: any) => {
    setActiveTab(ev.target.id);
  };
  ...
  const handleSearchSubmit = (ev: any) => {
    ev.preventDefault();
    ev.stopPropagation();
  };

  const filterLinks = (link: any) => {
    if (!searchTerm.length) {
      return true;
    }

    return link.title.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1;
  };

  const renderTablist = () => {
    return (
      <div className={styles.tablist} role="tablist" aria-label="Resources">
        <button
          role="tab"
          aria-selected="true"
          aria-controls="community-articles-blogs"
          id="article-blogs"
          className={activeTab === 'article-blogs' ? 'active' : ''}
          onClick={activateTab}
        >
        ...
        <button
          ...
          id="videos-media"
          className={activeTab === 'videos-media' ? 'active' : ''}
          onClick={activateTab}
        >
        ...
        <button role="tab" aria-selected="false" aria-controls="tools" id="tools" className={activeTab === 'tools' ? 'active' : ''} onClick={activateTab}>
        ...
        <button
          ...
          id="third-party"
          className={activeTab === 'third-party' ? 'active' : ''}
          onClick={activateTab}
        >
        ...
        <button role="tab" aria-selected="true" aria-controls="demos" id="demos" className={activeTab === 'demos' ? 'active' : ''} onClick={activateTab}>
          <span>Demos</span>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
  ...
  const renderItems = (arr: any[]) => {
    return arr.filter(filterLinks).map(link => {
      return (
        <li>
  ...
  return (
    <Layout title="Stencil Resources">
      <div className={styles.resourcesPage}>
        <ResponsiveContainer>
          <header>
            <h1 className="headline measure-md">
              <span>Resources</span> to help you get more out of Stencil
            </h1>
            <form className={styles.form} onSubmit={handleSearchSubmit}>
              <search-bar searchTerm={searchTerm} handleInput={handleSearchInput} placeholder={'Search Resources'}></search-bar>
            </form>
            <p>
              <a className={styles.addResource} href="https://github.com/ionic-team/stencil-site/edit/master/src/components/resources-page/resources-data.ts">
                + Add A Resource
              </a>
            </p>
          </header>
          ...
          <section>
            <div className={styles.tabs}>
              {renderTablist()}
              <div className={styles.tabpanel} tabindex="0" role="tabpanel" id="community-articles-blogs" aria-labelledby="article-blogs" hidden={activeTab !== 'article-blogs'}>
                <section className={styles.measureLg}>
                  <header>
                  ...
                  <ul className={styles.listUnstyled}>{renderItems(RESOURCES.BLOGS)}</ul>
                </section>
              </div>
              <div className={styles.tabpanel} tabindex="0" role="tabpanel" id="videos-media" aria-labelledby="videos-media" hidden={activeTab !== 'videos-media'}>
                <section className={styles.measureLg}>
                  <header>
                    <h2>Videos</h2>
                  </header>
                  <ul className={styles.listUnstyled}>{renderItems(RESOURCES.VIDEOS)}</ul>
                </section>
              </div>
              <div className={styles.tabpanel} tabindex="0" role="tabpanel" id="tools" aria-labelledby="tools" hidden={activeTab !== 'tools'}>
                <section className={styles.measureLg}>
                  <header>
                  ...
                  <ul className={styles.listUnstyled}>
                    {' '}
                    {renderItems(RESOURCES.TEMPLATES)}
                    {renderItems(RESOURCES.TOOLS)}
                  </ul>
                </section>
              </div>
              <div className={styles.tabpanel} tabindex="0" role="tabpanel" id="third-party-packages" aria-labelledby="third-party" hidden={activeTab !== 'third-party'}>
                <section className={styles.measureLg}>
                  <header>
                    <h2>3rd Party Components, Templates and Tools</h2>
                  </header>
                  <ul className={styles.listUnstyled}>{renderItems(RESOURCES.COMPONENTS)}</ul>
                </section>
              </div>
              <div className={styles.tabpanel} tabindex="0" role="tabpanel" id="demos" aria-labelledby="demos" hidden={activeTab !== 'demos'}>
                <section>
                  <header>
                    <h2>Demos</h2>
                  ...
                  <div className="demo-card-list">
                    {RESOURCES.DEMOS.filter(filterLinks).map(demo => {
