...
export default function NewsletterSignup () {
  ...
  const handleSubmit = useCallback((ev: any)  => {
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        setInlineMessage(json.inlineMessage)
        setIsLoading(false)
  ...
  }, [setIsValid, setIsLoading, setInlineMessage, setHasSubmitted])
      ...
      <div className={styles.form}>
        {hasSubmitted ? (
          <div className={styles.formMessage}>
            ...
            <div dangerouslySetInnerHTML={ { __html: inlineMessage} }></div>
          </div>
        ) : (
          <form onSubmit={e => handleSubmit(e)}>
            <div className={styles.formGroup}>
              <input
                ...
                required
              />
              <button className="btn btn--primary" type="submit" disabled={isLoading || hasSubmitted}>
                Subscribe
              </button>
            </div>
            {!isValid && <p className="error-message">{inlineMessage}</p>}
          </form>
        )}
      </div>
    </div>
