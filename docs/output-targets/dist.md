...
The `dist` output target generates a loader directory that exports an `setNonce`, `applyPolyfills` and `defineCustomElements` helper function when imported within an ESM context. This allows you to register all components of your library to be used in your project in e.g. an application setup script, e.g.:
